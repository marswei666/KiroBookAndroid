...
object SubscriptionManagerFactory {
    fun create(): SubscriptionManager {
        return when (BuildConfig.FLAVOR) {
            "direct" -> createDirectManager()
            "play" -> createPlayManager()
            else -> throw IllegalStateException("Unknown flavor: ${BuildConfig.FLAVOR}")
        }
    }

    private fun createDirectManager(): SubscriptionManager {
        return try {
            val clazz = Class.forName("com.minami_studio.kiro.data.subscription.DirectSubscriptionManager")
            clazz.getDeclaredConstructor().newInstance() as SubscriptionManager
        } catch (e: Exception) {
            throw IllegalStateException("DirectSubscriptionManager not found", e)
        }
    }
